...
class Config_Task2:
    # 1. Set the correct absolute path to dataset
    data_path = "/mnt/hdd2/task2/memsam/dataset_memsam_new/"
    
    # 2. This should be empty as 'videos' and 'annotations' folders are directly inside data_path
    data_subpath = "" 
    
    # 3. Change output paths to avoid overwriting other experiments
    save_path = "/mnt/hdd2/task2/memsam/checkpoints/Task2/"
    result_path = "/mnt/hdd2/task2/memsam/result/Task2/"
    tensorboard_path = "/mnt/hdd2/task2/memsam/tensorboard/Task2/"
    load_path = save_path + "xxx.pth" # Placeholder for resuming training
    save_path_code = "_"

    # --- Training Parameters ---
    workers = 4                         # Can be > 1 if system supports it
    epochs = 200
    batch_size = 4                      # Adjust based on GPU memory
    learning_rate = 1e-4
    momentum = 0.9
    
    # 4. Set the number of classes from class.json file
    # Open /mnt/hdd2/task2/memsam/dataset_memsam/class.json. 
    # If it says {"Custom": 23}, then set classes = 23: number of classes (including background)
    classes = 30
    
    img_size = 1024
    encoder_input_size = 1024
    
    # 5. Point to the filenames generated by preprocessing script
    train_split = "fold0_train_filenames.txt"
    val_split = "fold0_val_filenames.txt"
    test_split = "custom_test_filenames.txt"
    ...
    # 6. data format
    gray = "no"                         
    img_channel = 3                     # 3-channel RGB format
    
    # 7. Use a more general evaluation mode for custom datasets
    eval_mode = "Task2"            # the mode when evaluate the model, slice level or patient level
